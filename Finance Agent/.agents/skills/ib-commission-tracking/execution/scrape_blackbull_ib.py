#!/usr/bin/env python3
"""
scrape_blackbull_ib.py
======================
Playwright-based scraper for the BlackBull IB (Introducing Broker) Partner Portal.
...
Usage:
    ...
    python scrape_blackbull_ib.py --date-range 2026-01-01 2026-01-31
    python scrape_blackbull_ib.py --no-headless   # visible browser for debugging

Environment variables (loaded from workspace root .env):
    BLACKBULL_IB_EMAIL      - IB portal login email
    BLACKBULL_IB_PASSWORD   - IB portal login password
"""
...
#  Portal URLs (update if BlackBull changes its domain) 
LOGIN_URL = "https://partners.blackbull.com/login"
COMMISSION_REPORT_URL = "https://partners.blackbull.com/reports/commissions"
...
logger = logging.getLogger("blackbull_ib_scraper")
...
def _take_error_screenshot(page, label: str = "error") -> Path:
    ...
    path = SCREENSHOTS_DIR / f"blackbull_{label}_{ts}.png"
...
def login(page, email: str, password: str) -> None:
    """
    Log into the BlackBull IB partner portal.

    UPDATE SELECTORS BELOW if BlackBull changes their login page HTML.
    """
    ...
    SUBMIT_SELECTOR = 'button[type="submit"], input[type="submit"], .login-btn, .btn-login'
    # 
...
def _handle_2fa(page) -> None:
    """
    If a 2FA / OTP prompt appears, pause and wait for manual completion.
...
def navigate_to_commissions(page, date_start: str, date_end: str) -> None:
    ...
    DATE_START_SELECTOR = 'input[name="startDate"], input[name="date_from"], input[name="from"], #startDate'
    DATE_END_SELECTOR = 'input[name="endDate"], input[name="date_to"], input[name="to"], #endDate'
    APPLY_FILTER_SELECTOR = 'button:has-text("Apply"), button:has-text("Filter"), button:has-text("Search"), button:has-text("Go")'
    # 
...
def scrape_commission_data(page, date_start: str, date_end: str) -> dict:
    ...
    TOTAL_COMMISSION_SELECTOR = (
        ...
        '.card:has-text("Commission") .value, '
        '.stat-card:has-text("Commission") .number'
    )
    CLIENT_COUNT_SELECTOR = (
        '[data-testid="client-count"], '
        '.client-count .value, '
        '.card:has-text("Client") .value, '
        '.stat-card:has-text("Client") .number'
    )
    TOTAL_LOTS_SELECTOR = (
        '[data-testid="total-lots"], '
        '.total-lots .value, '
        '.card:has-text("Volume") .value, '
        '.card:has-text("Lots") .value, '
        '.stat-card:has-text("Lots") .number'
    )
...
def export_data(data: dict) -> tuple[Path, Path]:
    ...
    ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    base_name = f"blackbull_commissions_{ts}"
    ...
    # Also write a "latest" copy for the aggregator
    latest_json = RESOURCES_DIR / "blackbull_commissions_latest.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Scrape BlackBull IB Partner Portal commissions."
    )
    parser.add_argument(
        "--date-range",
        nargs=2,
    ...
    email = os.getenv("BLACKBULL_IB_EMAIL")
    password = os.getenv("BLACKBULL_IB_PASSWORD")
    if not email or not password:
        logger.error(
            "Missing credentials. Set BLACKBULL_IB_EMAIL and BLACKBULL_IB_PASSWORD "
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            log_task("Finance", "Scraped BlackBull IB commission data", "Done", "Medium", f"Exported {date_start} to {date_end} commissions to CSV and JSON")
            return
